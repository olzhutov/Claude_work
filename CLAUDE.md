...
Для всей аналитики, оценки объектов и парсинга данных использовать навык:

```
./.agents/skills/cre-valuation/
```

**README.md** навыка содержит полную методологию:
- `[B1]` Сравнительный подход — скидки на торг, матрица корректировок, P10–P90
- `[B2]` Доходный подход — NOI, Cap Rate, WALT, IRR; ориентиры Украина 2024-25
- `[B3]` Согласование результатов (веса 40-60%)
- `[C1]` Поиск аналогов — правила отбора, флаги аномалий
- `[C2]` Обработка CSV/XLSX/JSON — очистка, дубли, нормализация валюты
- `[C3]` Извлечение признаков из текстов объявлений

**Языковой стандарт:** рабочая переписка — русский; все отчёты, документы, справки — украинский.
